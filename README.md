...
GCEでNode-REDを動かすためにGCPでいくつか設定する
...
2～6の手順は[バッチファイル実行で自動化できる](#vmインスタンス生成を自動化)  
GCEの起動・停止については「[VMインスタンスの起動/停止を管理する](#vmインスタンスの起動停止を管理する)」に記述している
...
VMインスタンスの生成を自動化出来るようにまとめたフォルダを用意した
